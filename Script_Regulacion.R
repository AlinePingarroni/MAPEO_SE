# ========================
# VISUALIZACIÓN DE SERVICIOS DE REGULACIÓN
...
# 1. Cargar el raster de regulación
r_regulacion <- rast("REGULACION.tif")

# 2. Convertir a data.frame para ggplot
df_reg <- as.data.frame(r_regulacion, xy = TRUE, na.rm = TRUE)
names(df_reg)[3] <- "PUNTOS"

# 3. Obtener bounding box del raster y prepararlo para OSM
ext_reg <- ext(r_regulacion)
poly_reg <- as.polygons(ext_reg)
sf_reg <- st_as_sf(poly_reg)
st_crs(sf_reg) <- 32614  # UTM zona 14N

# Transformar a WGS84 para usar con OpenStreetMap
sf_reg_wgs <- st_transform(sf_reg, 4326)

# Expandir el bbox para que aparezcan calles alrededor
bbox_exp <- st_bbox(sf_reg_wgs)
...
ggplot() +
  ...
  geom_raster(data = df_reg, aes(x = x, y = y, fill = PUNTOS), alpha = 0.8) +
  scale_fill_viridis_c(name = "Valor de Importancia", option = "D") +
  coord_sf(crs = 32614) +
  theme_minimal() +
  labs(title = "Servicios de Regulación en la FES-Iztacala",
       subtitle = "Mapeo participativo") +
  annotation_scale(location = "bl", width_hint = 0.2) +
  annotation_north_arrow(location = "tr", which_north = "true", style = north_arrow_fancy_orienteering())

# Guardar el mapa como imagen PNG (300 dpi)
ggsave("mapa_regulacion_fes.png",
