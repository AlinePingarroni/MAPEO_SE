# ========================
# VISUALIZACIÓN DE SERVICIOS CULTURALES
...
# 1. Cargar el raster de servicios culturales
r_cultural <- rast("CULTURAL.tif")

# 2. Convertir a data.frame para ggplot
df_cult <- as.data.frame(r_cultural, xy = TRUE, na.rm = TRUE)
names(df_cult)[3] <- "PUNTOS"

# 3. Obtener bounding box del raster y prepararlo para OSM
ext_cult <- ext(r_cultural)
poly_cult <- as.polygons(ext_cult)
sf_cult <- st_as_sf(poly_cult)
st_crs(sf_cult) <- 32614  # UTM zona 14N

# Transformar a WGS84 para usar con OpenStreetMap
sf_cult_wgs <- st_transform(sf_cult, 4326)

# Expandir el bbox para que aparezcan calles alrededor
bbox_exp <- st_bbox(sf_cult_wgs)
